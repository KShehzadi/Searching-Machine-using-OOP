...
CName& CName::operator =(const CName &src)
{
  ...
  strcpy(this->first, src.first);
  delete [] this->mid;
  this->mid = new char[strlen(src.mid) + 1];
  strcpy(this->mid, src.mid);
  delete [] this->last;
  this->last = new char[strlen(src.last) + 1];
  strcpy(this->last, src.last);
  return *this;
}
